...
```bash
mkdir -p "{{SYNC_ROOT}}/skills/secondbrain-research-digest"
# 請使用本文文末「內建 Skill 完整安裝內容」；不需要額外複製舊版獨立 skills 子目錄。
test -f "{{SYNC_ROOT}}/skills/secondbrain-research-digest/SKILL.md" && echo "secondbrain-research-digest installed"
```
...
可以用任一支援排程的 Agent automation，或手動請當前 Codex、Claude、AntiGravity 執行。三者使用同一個 prompt 契約；排程 UI 與啟動指令另記 adapter。建議 prompt：
...
本節是自含式安裝區塊。這個序號項目會安裝：`secondbrain-research-digest`。
...
mkdir -p "$(dirname "{{SYNC_ROOT}}/skills/secondbrain-research-digest/SKILL.md")"
cat > "{{SYNC_ROOT}}/skills/secondbrain-research-digest/SKILL.md" <<'AGENT_LAZYPACK_SECONDBRAIN_RESEARCH_DIGEST_SKILL_MD_0E95F5A366'
---
name: secondbrain-research-digest
description: "觸發：整理研究、比較資訊、做決策、把零散素材轉成 Obsidian 可用的 Markdown 筆記、查詢或彙整 Secondbrain 內容。輸出精簡繁中 Markdown 加結構化決策／資料區塊。"
metadata:
...
- Secondbrain vault: `OBSIDIAN_VAULT`; for this user, `{{OBSIDIAN_VAULT}}`
...
AGENT_LAZYPACK_SECONDBRAIN_RESEARCH_DIGEST_SKILL_MD_0E95F5A366

# secondbrain-research-digest/references/research-note-template.md
mkdir -p "$(dirname "{{SYNC_ROOT}}/skills/secondbrain-research-digest/references/research-note-template.md")"
cat > "{{SYNC_ROOT}}/skills/secondbrain-research-digest/references/research-note-template.md" <<'AGENT_LAZYPACK_SECONDBRAIN_RESEARCH_DIGEST_REFERENCES_RESEARCH_NOTE_TEMPLATE_MD_F0FCC8341A'
---
...
AGENT_LAZYPACK_SECONDBRAIN_RESEARCH_DIGEST_REFERENCES_RESEARCH_NOTE_TEMPLATE_MD_F0FCC8341A

test -f "{{SYNC_ROOT}}/skills/secondbrain-research-digest/SKILL.md" && echo "secondbrain-research-digest installed for Codex, Claude, and AntiGravity"
````
