...
1. 過去一個月，有哪件工作你反覆做了三次以上？
2. **有哪件事你做得很好，卻每次都得重新想一遍，或很難交接給別人？**
3. 做這件事時，你會打開哪些工具、翻哪些資料，最後交出什麼？
4. 哪個環節讓 AI 先打草稿就能省下大半力氣？哪個環節非你本人決定不可？
5. 萬一做錯，最壞會出什麼事？（例如對外報價、個資外流、合約條款、公開發布）

**第 2 題是關鍵。**「很難交接給別人」代表存在值得萃取的隱性判斷；容易教的通常只需要一張檢查表。
...
- Never write a fixed option that pretends to be a text field, such as「輸入自訂名字」or「在下方輸入框填寫」. In most picker UIs clicking an option submits its label text as the answer; only the tool's built-in Other field accepts typing. When free text is needed, keep only real fixed options and tell the user to use Other, or ask a plain-text question. Apply the same check to every user-facing interview question a new or updated skill contains.
