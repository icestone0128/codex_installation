...
5. 先確認是「只產生計畫」、「稽核現況」或「實際安裝」，再用優先順序選最輕且穩定的路線：
   - Google Workspace 個資服務：Gmail／Calendar／Drive／Docs／Sheets／Slides／Tasks／Classroom 預設走本機 Google Workspace MCP，不走 connector（見全域規則〈Google 服務接取預設路由〉）。要加服務或擴權，改 MCP 的權限設定並重新完成 OAuth 同意，且須使用者明確要求；MCP 做不到或使用者指定時才改用 connector，並回報實際路線。API key 不能讀取私人 Gmail／Drive／Classroom 資料。
   - Agent 原生 connector／plugin／MCP：適合 GitHub 等當前 Agent 已支援的工具（Google 服務除外，見上一條）；必須逐 Agent 驗證，不假設目錄或 OAuth session 共用。
...
| 服務 | 建議通道 | 鑰匙 / 授權 | 注意 |
|---|---|---|---|
| Obsidian | 本機檔案優先；App 層操作（搜尋、tasks、設定）用官方 CLI `obsidian`；兩者都不適合才用 MCPVault 類 MCP | 免 API key，本機 vault 路徑 | vault 路徑要明確，不掃無關資料夾。CLI 連的是執行中的 Obsidian：App 要開著，且要在 設定 > 一般 > 進階 開啟 Command line interface，否則每個指令都回報未啟用。請使用者自己開，不代改 App 設定。 |
| GitHub | Codex / GitHub connector 或 `gh` CLI | OAuth / provider login | connector 與 CLI auth 分開驗證。 |
| Google Drive / Gmail / Calendar / Docs / Sheets / Slides | 本機 Google Workspace MCP | 經 MCP 的 OAuth | API key 不能讀私人 Workspace 資料。connector 只作回報過的備援。 |
| Google Classroom | 本機 Google Workspace MCP（工具層級有涵蓋時）；否則先問再選其他路線 | OAuth | 發作業、學生名單、繳交狀態需謹慎 scope。 |
...
| Notion / 其他 SaaS | connector / plugin 優先，其次官方 MCP / API | OAuth 或 API token | 寫入前確認 workspace、page、database schema。Notion 讀取與簡單查詢走 connector／MCP 即可；多欄位、關聯、批次更新這類複雜資料庫寫入，建議改用 REST API 搭配 `{{SECRETS_DIR}}` 裡的 token，先在測試資料庫試。 |
...
---
name: tool-integration-workflow
description: "規劃、盤點、整合、安裝、連接或驗證外部工具、API、CLI、MCP server、connector 或服務（Firecrawl、Notion、Slack、Linear、Gmail、GitHub、Drive）。支援只規劃與實作兩種模式，不把 secret 寫進 repo。"
metadata:
  short-description: External tool integration workflow
---
...
5. Treat Google Workspace personal-data services as a special case. Gmail, Calendar, Drive, Docs, Sheets, Slides, Tasks, and Classroom need OAuth for personal data; an API key cannot read a user's private Google Workspace data. Follow `core-rules.md`〈Google 服務接取預設路由〉: route every Google service through the local Google Workspace MCP first, not a connector. Add a service or widen scope by changing that MCP's permissions and re-running its OAuth consent, only when the user explicitly asks. Fall back to a connector only when the MCP cannot do the job or the user names the connector, and report which route was used.
6. Prefer the lightest stable route that fits the task:
   - Google services: the local Google Workspace MCP (rule 5), ahead of every other route.
...
- Gmail, Google Calendar, Google Drive, Docs, Sheets, Slides, Tasks, and Google Classroom personal data require OAuth.
- A single API key cannot read a user's private Gmail, Drive, Calendar, or Classroom data.
- **Default route is the local Google Workspace MCP, not a connector** (`core-rules.md`〈Google 服務接取預設路由〉). One maintained MCP covers the whole Google suite, keeps the permission boundary in one wrapper, and behaves the same across Codex, Claude, and AntiGravity.
- Need another Google service or a wider scope? Change the MCP's `--permissions`／`--tool-tier` in its wrapper and the matching LazyPack Item 02 source, then redo OAuth consent. Only on the user's explicit request; never open a new connector for it.
- Use a connector only when the MCP cannot do the job or the user names the connector; say which route was actually used.
- Existing connectors are not switched off by this rule.
- For classroom operations such as creating assignments, review scopes carefully before granting write access.
...
1. Codex connector / plugin
   ...
   - Typical example: GitHub. Google services are the exception and go through the local Google Workspace MCP (see above).
2. Official CLI
...
| --- | --- | --- | --- |
| Obsidian | Local filesystem first; official CLI `obsidian` for app-level actions (search, tasks, settings); MCPVault-style MCP only if neither fits | No API key; local vault path | Keep vault path explicit and avoid scanning unrelated folders. The CLI talks to the running Obsidian app: the app must be open and Settings > General > Advanced > Command line interface must be on, otherwise every command prints "Command line interface is not enabled" (verified 2026-09-17, macOS, `/opt/homebrew/bin/obsidian` → `Obsidian.app/Contents/MacOS/obsidian-cli`). Ask the user to enable it; do not flip app settings yourself. |
...
| Google Drive/Gmail/Calendar/Docs/Sheets/Slides | Local Google Workspace MCP | OAuth via the MCP | API key cannot read private Workspace data. Connector only as a reported fallback. |
| Google Classroom | Local Google Workspace MCP if the tool tier covers it; otherwise ask before any other route | OAuth | Writing assignments or reading student data needs careful scope review. |
...
| Notion/SaaS | Connector/plugin first, then official MCP/API | OAuth or API token | Invite integration to the target workspace/page/database before testing. For Notion, reads and simple queries are fine through a connector or MCP; for complex database writes (many properties, relations, bulk updates) prefer the REST API with a token from `~/.codex/secrets/`, which gives exact control and clearer errors. Test on a scratch database first. |
...
---
name: cli-anything
description: "Use to build, refine, test, validate or list CLI-Anything harnesses for GUI applications or source repositories, keeping the generated Python harness format."
---
