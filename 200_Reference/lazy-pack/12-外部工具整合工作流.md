...
upstream tracking: latest commit of the default branch (updated on every re-run)
...
- 安裝器以 detached HEAD 指向 `origin/HEAD`（上游預設分支最新 commit）；這是正常狀態，不是錯誤。
...
---
name: tool-integration-workflow
description: "要規劃、盤點、整合或驗證外部工具、API、CLI、MCP server 或 connector 時使用；secret 不進 repo。"
metadata:
  short-description: External tool integration workflow
---
...
- Source checkout: `{{CODEX_HOME}}/python-tools/CLI-Anything`
- Upstream tracking: the installer checks out the latest commit of the upstream default branch on every run.
- Three-agent shared skill: `{{SYNC_ROOT}}/skills/cli-anything`
- CLI-Hub venv: `{{CODEX_HOME}}/python-tools/cli-anything-hub/.venv`
...
- The installer leaves the source checkout in detached HEAD at `origin/HEAD`; this is expected, not an error.
...
---
name: cli-anything
description: "Use to build, test or list CLI-Anything harnesses for GUI apps or source repos."
---
