...
> 版本：2026-06-03 三 Agent 共用版
> 用途：安裝 `netlify-deploy` 全域 skill，並為 Codex、Claude、AntiGravity 分別記錄官方 Netlify MCP 原生 adapter；Netlify CLI 作為三者共用 fallback。
> 成品：下載者可直接使用本文文末「內建 Skill 完整安裝內容」建立 `{{SYNC_ROOT}}/skills/netlify-deploy/`。
...
- 三 Agent 共用全域 skill：`{{SYNC_ROOT}}/skills/netlify-deploy/SKILL.md`。

## 三 Agent 相容化調整

- 保留來源 repo 的 Clasp + Netlify 閉環部署思路與踩坑清單。
- 將來源中的 agent／tool 專屬名稱轉成共用 Netlify 服務契約，再分別記錄 Codex、Claude、AntiGravity 的 MCP adapter。
- 正式 MCP route 改用 Netlify 官方 `@netlify/mcp`。
- 不內嵌 Netlify PAT、Google OAuth token、GitHub token 或任何 private key。
- 記錄兩種 npm／CLI 權限路線：共用短期 fallback 可用 `NPM_CONFIG_CACHE=/private/tmp/npm-cache` 或一般 Terminal 驗證；Codex 長期 adapter 則在 sandbox writable roots 補入 `~/.npm`、`~/Library/Preferences/netlify` 與 `~/.clasprc.json`。Claude／AntiGravity 如有沙箱限制，依它們當前版本的原生權限設定處理。

## Agent Execution Notes

- Codex adapter：將官方 MCP 設定寫入 `{{CODEX_CONFIG}}`，必要時套用本文的 Codex sandbox writable-roots 修正。
- Claude adapter：依當前 Claude 安裝版本的原生 MCP 設定與官方 help 新增同一 `@netlify/mcp`；不直接複製 Codex TOML。
- AntiGravity adapter：依當前 AntiGravity 安裝版本的原生 MCP 設定與官方 help 新增同一 `@netlify/mcp`；不直接複製 Codex TOML。
- 通用 fallback：三 Agent 都可使用 Netlify CLI 進行登入、status、link、deploy 與 troubleshooting，實際部署前依然要取得使用者授權。

## 官方建議的 MCP / CLI 取用原則

- MCP 是 Agent 原生 route：用於讓 Codex、Claude、AntiGravity 讀取 Netlify user／team／site 狀態、建立或管理 project、設定環境變數或 secrets、部署已確認的輸出資料夾；三者各自使用原生 MCP adapter。
...
## Netlify MCP 安裝（Codex adapter 範例）
...
修改後重載 Codex MCP tools。Claude 與 AntiGravity 依前述 Agent Execution Notes 寫入各自原生設定並重載。

## Netlify CLI 安裝

Netlify CLI 用來做本機登入、帳號狀態檢查與部署 troubleshooting。MCP 是各 Agent 的原生工具通道；CLI 是三者共用 fallback。
...
1. 依 Agent Execution Notes 將 `netlify` MCP 加入 Codex、Claude、AntiGravity 各自原生設定；上方 TOML 僅是 Codex adapter。
2. 打開本文文末「內建 Skill 完整安裝內容」。
3. 把整段安裝腳本複製到自己的環境執行。
4. 執行前先把 `{{CODEX_HOME}}` 替換成自己的 Codex 設定資料夾，例如 `{{HOME}}/.codex`。
5. 對三 Agent 分別重載 MCP 與 skill 清單。
...
test -f "{{SYNC_ROOT}}/skills/netlify-deploy/SKILL.md" && echo "netlify-deploy SKILL.md ok"
test -f "{{SYNC_ROOT}}/skills/netlify-deploy/references/source-adaptation.md" && echo "source adaptation ok"
test -f "{{SYNC_ROOT}}/skills/netlify-deploy/references/source-original-readme.md" && echo "source original record ok"
test -f "{{SYNC_ROOT}}/skills/netlify-deploy/references/clasp-netlify-pattern.md" && echo "clasp pattern ok"
```

對三 Agent 分別重載後，請當前 Agent 檢查是否有可用的 Netlify MCP tools。第一次部署前先做 read-only account／status 類動作，再建立 site 或 deploy。
...
| 當時的 Codex 相容性 | 2026-06-03 當時只驗證 Codex adapter；此單一環境紀錄已被 2026-07-20 三 Agent 契約取代，現行版必須同時保留 Claude 與 AntiGravity adapter |
...
- [ ] Codex、Claude、AntiGravity 各自原生 MCP 設定都已建立或明確記錄 CLI fallback；Codex adapter 在 `{{CODEX_CONFIG}}` 有 `[mcp_servers.netlify]`。
...
- [ ] `{{SYNC_ROOT}}/skills/netlify-deploy/SKILL.md` 存在。
- [ ] references 三個檔案存在。
- [ ] Codex、Claude、AntiGravity 重載後能看到 Netlify MCP tools；若某原生通道不可用，已實測 Netlify CLI fallback。
...
---
name: netlify-deploy
description: Use when installing, verifying, or using Netlify MCP or CLI from Codex, Claude, or AntiGravity, deploying static/frontend projects to Netlify, or building a Clasp Google Apps Script plus Netlify frontend deployment workflow.
metadata:
...
|---|---|---|
| Shared skill | `{{SYNC_ROOT}}/skills/netlify-deploy/SKILL.md` and the three `references/` files exist through all native entrypoints | Skill package is readable by Codex, Claude, and AntiGravity |
...
6. Deploy frontend through the active Agent's Netlify MCP adapter or the shared Netlify CLI, not through Apps Script.
...
13. Run a local frontend preview and check desktop/mobile layout. If Playwright
    cannot launch Chromium because of the active Agent's process permissions,
    rerun the same browser verification through an approved permission path or
    shared browser route rather than skipping visual verification.
