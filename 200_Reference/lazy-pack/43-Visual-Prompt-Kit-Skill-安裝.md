...
先呈現文章洞察，再**一次問完**風格方向。不要拆成兩輪互動：從風格庫挑候選與
自由描述偏好是同一個問題的兩種答法。

**絕對不要一次列出整個風格庫。** 三種回法同時給：
...
兩者可以混用（選了編號再補一句「但要更暗一點」）。

**同一輪一併問人物選項**（版位支援時）：不放人物／預留真人空位／放角色插畫。
不要為了人物再問第二輪。
...
先把洞察攤給使用者看，再一次問完風格方向。**只問一輪**——風格庫候選與自由描述
偏好是同一個問題的兩種答法，不要拆成兩次互動。
...
**兩個問題一起問完，不要分兩輪。** 風格可以混用回答（選了編號還能再補一句
「但要更暗一點」，兩者都收進 Design Anchor）；人物選項則三選一。
