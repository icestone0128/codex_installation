...
> 2026-08-26 建立、2026-08-27 更新。來源為第三方 `agent-dev-coach` 套件，其五關骨架來自
> Matt Pocock 公開的 `mattpocock/skills` 開發流程（grill → spec → tickets → implement → review）。
> 本 Item 改寫為 Codex／Claude／AntiGravity 三 Agent 共用版本，加上教練話術、HC 標籤與同意閘門，
> 並修掉原版在學員專案內無法解析腳本路徑的問題。**底層方法不是原創，教學封裝才是**——
> 這也是它與 Item 40 同源卻不重複的原因。
...
**Claude 有兩種安裝面，別裝錯邊**（這一項最常卡住第一次安裝的人）：

| 你用的是 | 怎麼裝 |
| :-- | :-- |
| Claude Code（終端機） | 就是上面這條路：檔案系統 `~/.claude/skills/agent-dev-coach` |
| Claude 桌面版／網頁版（含 Cowork） | 到 claude.ai → 左下角你的名字 → Customize → Skills → ＋ → Create skill → Upload a skill，上傳 skill 資料夾打包成的 zip（不用解壓縮） |

桌面版與網頁版讀的是**你 claude.ai 帳號裡的技能**，不會去讀你電腦上的 `~/.claude/skills/`——
放進去也不會生效。Codex 與 AntiGravity 沒有這個分岔，都走檔案系統入口。
...
**被叫起來的第一件事是看 `state.json` 在不在**，這決定走哪條路：

- **不存在**＝全新專案：走開場自我介紹，問「你手上現在有什麼？」，從第 1 關開始。
- **已存在**＝學員回來續跑：不重新自我介紹、不再問手上有什麼（檔案裡就寫著），
  直接回報做到第幾關、上次出口檔是什麼、有沒有 `[待決]` 未結，再問要從哪接。

配套的兩個日常用語，學員不需要記指令：

- 說「**先到這裡**」＝暫停。立刻停、把已談出的內容落檔，**不准再追問要不要進下一關**，
  並提一次要不要順手打包 PRD。想停還被追問下一關，是這個流程最容易讓人不想再用的失誤。
- 說「**繼續**」＝接 `state.json` 記的下一關，但仍要問過一次才動。
...
**被叫起來時的第一件事：先看 `.agent-flow/state.json` 在不在。** 這決定你要走哪一條路，不要跳過。

**已經存在＝學員回來續跑**，不要重新自我介紹，也不要再問「你手上現在有什麼」（他手上有什麼，檔案裡就寫著）：

1. 讀 `state.json` 與已存在的交付物，用兩三行回報：做到第幾關、上次的出口檔是什麼、有沒有 `[待決]` 未結
2. 問他：**「要從第 N 關接下去，還是先看／改前面的東西？」**——由他決定，不要自己往前推
3. 他說「繼續」而沒指定關卡時，就接 `state.json` 記的下一關，但仍要問過一次才動

**不存在＝全新專案**，才走下面的開場（學員還沒說要做什麼、或只說了一句模糊的話）：
...
| 「先到這裡」「今天先這樣」「之後再說」 | **暫停**（見下）——不是換關 |
| 「繼續」「接著上次」「我回來了」 | 讀 `.agent-flow/state.json` 續跑（見上） |

**學員表達想停時**（「先到這裡」「今天先這樣」）：**立刻停，不要再問要不要進下一關。** 把目前這一關已談出來的東西寫進對應的 `.agent-flow/` 檔案與 `state.json`，用兩三行告訴他停在哪、下次說「繼續」就接得回去，並提一次「要不要順手打包成 PRD？」——他說要才做。想停還被追問下一關，是這個 skill 最容易讓人不想再用的失誤。

**學員問的事不屬於任何一關時**（例如問這個工具怎麼安裝、能不能在別的平台用、某個名詞是什麼意思）：照常好好回答，**但回答完必須主動問一次**「要不要開始第 1 關？」——不要答完就停，學員不會知道教練什麼時候該上工。**唯一例外是上一段的「想停」**：那是他已經表態了，追問等於沒在聽。
...
五關的骨架來自 Matt Pocock 公開的 `mattpocock/skills` 開發流程（grill → spec → tickets → implement → review），
本 skill 把它改寫成中文教學版：加上教練話術、密涅瓦思考習慣（HC）標籤、`.agent-flow/` 狀態機，
以及「每關必須明確徵得同意才能前進」的閘門。底層方法不是原創，教學封裝才是。

同一批方法在全域另有一套非教學版本（見下一節的 `engineering-methods`）；兩者刻意併存，不互相取代。
...
- **使用者要自己動手、不想被帶流程**：走 `engineering-methods` 那一套（`grilling`、`to-spec`、`to-tickets`、`implement`、`code-review` 各自獨立呼叫、順序自由、沒有前進閘門）。本 skill 是**帶人**的單一連續流程，多了教練話術、HC 標籤、狀態機與同意閘門。同一批底層方法，兩種使用情境——**可以接力**：用這裡想清楚需求與切票，日常實作與審查回到 `engineering-methods`。
...
<div class="page">
  ...
  <div class="footer-card">
    ...
    <div class="footer-note">進度卡只管路由；規格內容以 <code>spec.json</code> 為準。進度存在專案的 <code>.agent-flow/</code>，下次開新對話直接說「繼續」即可接回，不需要上傳任何檔案。</div>
  </div>
</div>
</body>
</html>
