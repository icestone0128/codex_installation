...
| 自動剪掉停頓 | 口播片停頓多，人工粗剪費時 | Auto-Editor | 「保留原比例，用智能剪輯去除停頓，先產預覽給我確認」 |
...
| Auto-Editor | 已納入 Item 34 | 安裝時取官方 GitHub 最新 standalone，並以 release 公布的 SHA-256 驗證；避開落後的 user-level Python 版本。 |
...
| MacWhisper | Item 29 的 macOS 選項 | 已驗證 `mw` CLI；每次升級或不同授權先跑 `macwhisper-cli --help`，輸出仍需驗證時間碼。 |
