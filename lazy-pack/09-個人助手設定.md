...
本節取代舊版 `對應序號文件的內嵌 Skill 區塊：` 子目錄。這個序號項目會安裝：`arry-assistant`。
...
6. 若使用者要新增 skill，先判斷歸屬：
   - 跨專案可重用或需要全域觸發：放 `/Users/arrywu/.codex/skills/<skill-name>`，並同步 `codex_installation/lazy-pack/<對應序號文件的內嵌 Skill 區塊>` 與 Obsidian 全域 Skills 索引。
