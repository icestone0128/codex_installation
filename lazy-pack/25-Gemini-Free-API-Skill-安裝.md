...
- 這版定位：把原本「設定 Gemini 免費 API」改成 Codex App 可用的全域 skill，並加入 API key 安全儲存、Free Tier / Paid Tier 判斷、模型選擇與低風險驗證流程。
...
| 5 | 預設模型改為 `gemini-3.5-flash`；若使用者所在專案尚未開放，才退回 `gemini-2.5-flash` 驗證。 |
...
觸發語意包含：Gemini API、Google AI Studio、Gemini Free Tier、GEMINI_API_KEY、AI Studio API key、Gemini 3.5 Flash、Gemini 2.5 Flash、API 收費、API billing。

## 預設工作流程

1. 先判斷使用者是要設定 key、驗證 key、檢查收費，還是把專案改成呼叫 Gemini。
2. 若涉及目前方案或模型可用性，優先查 Google AI Studio 專案的 Billing Tier 與 Models / Rate limits 頁面。
...
Free Tier 可免費開始使用，但有模型、速率、功能與資料使用條款限制。若專案升級或連到 billing account，可能進入 paid / prepay / postpay 流程。
...
description: Use when the user asks to set up, verify, troubleshoot, or safely use Google AI Studio Gemini API Free Tier in Codex projects, including GEMINI_API_KEY storage, Gemini 3.5 Flash or 2.5 Flash verification, billing-tier checks, and backend-safe API integration without exposing secrets.
metadata:
...
- `GEMINI_API_KEY`;
- Gemini 3.5 Flash, Gemini 3 Flash, or Gemini 2.5 Flash in a Codex-built tool;
...
2. If current billing or model availability matters, inspect Google AI Studio
   directly or ask the user to confirm the project's Billing Tier.
...
Fallback if the project does not have Gemini 3.5 Flash yet:
...
Expected result: JSON response containing `OK`. If the response is 401, check
the key. If it is 403, check project/API permissions or billing state. If it is
429, wait or check rate limits.
...
- Keep model name configurable, with `gemini-3.5-flash` as the preferred default
  and `gemini-2.5-flash` as a compatibility fallback.
...
Model names, free-tier limits, rate limits, billing plans, and pricing can
change. For cost, model availability, or "latest" questions, verify against
official Google AI documentation or the user's current AI Studio project before
...
- The project is not linked to a paid billing account unless the user intends
  paid usage.
- The model used by the request is available in that tier.
...
| 404 model not found | Model not available for project or endpoint | Try fallback model or verify current model list |
