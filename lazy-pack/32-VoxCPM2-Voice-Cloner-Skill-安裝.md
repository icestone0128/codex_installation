...
- runtime 可放在私有全域助手資料層，例如 `{{SYNC_ROOT}}/runtimes/voxcpm2-voice-cloner`，再由 `{{CODEX_HOME}}/voxcpm2-voice-cloner` symlink 指向它；Hugging Face 模型權重固定放在 runtime 的 `model-cache/`，供第二台電腦沿用。
...
{{SYNC_ROOT}}/runtimes/voxcpm2-voice-cloner/.venv/
{{SYNC_ROOT}}/runtimes/voxcpm2-voice-cloner/model-cache/
{{SYNC_ROOT}}/assets/arry-voice-profiles/Arry/
<project-root>/100_Todo/projects/voxcpm2-voice-cloner/output/
```

只有第一個路徑是可攜 Skill package；runtime 與模型快取放在私有全域助手資料層，reusable voice profile 放全域助手資產，生成輸出放當次專案資料夾且排除 Git。
...
若使用跨電腦共享 runtime，先建立全域 runtime symlink：

```bash
mkdir -p "{{SYNC_ROOT}}/runtimes"
if [ ! -e "{{CODEX_HOME}}/voxcpm2-voice-cloner" ]; then
  ln -s "{{SYNC_ROOT}}/runtimes/voxcpm2-voice-cloner" "{{CODEX_HOME}}/voxcpm2-voice-cloner"
fi
```

若 `{{SYNC_ROOT}}/runtimes/voxcpm2-voice-cloner/model-cache/` 已由另一台電腦同步完成，下一步不應重新下載模型；若 `.venv` 因平台或 Python 路徑不相容，可重跑 setup，但保留 `model-cache/`。
...
- Runtime data: `${VOXCPM2_HOME:-$HOME/.codex/voxcpm2-voice-cloner}`. For Arry, this is a symlink to `codex_symlink/runtimes/voxcpm2-voice-cloner` so the venv, model cache, and runtime settings can sync across computers.
...
Never copy model weights, generated audio, or the runtime virtual environment into LazyPack, Obsidian, Git, or the global skill package. For cross-computer reuse, put the runtime under the user's private assistant data layer and point `$VOXCPM2_HOME` or a symlink at it. For Arry, the canonical runtime is `codex_symlink/runtimes/voxcpm2-voice-cloner`. Reusable personal voice profiles may live in the user's private assistant asset layer; for Arry, the canonical profile folder is `codex_symlink/assets/arry-voice-profiles/Arry/`. Project outputs should live under the project that requested generation and remain gitignored when private.
...
For Arry's cross-device setup, ensure this symlink exists before running setup or generation:

```bash
ln -s "{{SYNC_ROOT}}/runtimes/voxcpm2-voice-cloner" "$HOME/.codex/voxcpm2-voice-cloner"
```
...
export VOXCPM2_OUTPUT_DIR="$PWD/100_Todo/projects/voxcpm2-voice-cloner/output"
```
...
For cross-computer reuse, the runtime root may live in the user's private assistant data layer rather than only on one machine. Arry's canonical runtime is:

```text
codex_symlink/runtimes/voxcpm2-voice-cloner/
├── .venv/
├── uv-cache/
├── model-cache/
├── recordings/
├── voices/ -> profile symlinks or local fallback
└── output/ -> current project output symlink or local fallback
```

On each computer, `~/.codex/voxcpm2-voice-cloner` should be a symlink to that shared runtime folder. The `.venv` is synced for convenience, but if Python path, macOS architecture, or compiled packages differ on another machine, rerun `scripts/setup_runtime.sh` against the same shared runtime; keep `model-cache/` so the multi-GB model does not need to download again.
