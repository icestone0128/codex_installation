...
```text
01：基礎 plugins / connectors 檢查，例如 Notion、PDF、Browser plugin；不重複安裝全域 `pdf`、`browser` 或 `playwright` skill
...
| 12 | PDF / Browser plugin | [[01-Codex-必裝-Skills-與-Plugins]] | 使用 Codex App 內建 plugin 能力；PDF 與 Browser 列為必裝檢查，不要重複安裝全域 `pdf`、`browser` 或 `playwright` skill |
...
| 23 | 其他內容製作類 skills | 對應序號文件 | 視需求安裝 |
