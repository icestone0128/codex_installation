...
4. Create or update the project root `AGENTS.md` with:
   ...
   - optional personal assistant integration: use the shared global personal-assistant skill and shared core layer, plus project-local `100_Todo/` and `200_Reference/`
...
6. Create the project-local data layer if missing:
   ...
   - `000_Agent/project-memory/` only when the project needs project-specific assistant memory separate from the global Arry core
...
- If the user asks for project-local assistant memory, create it under `000_Agent/project-memory/` and document how it syncs to the project cockpit.
